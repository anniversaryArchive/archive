<template>
  <!-- 이미지 -->
  <q-file
    v-if="field.type === 'image' || field.type === 'images'"
    name="data_file"
  ...
  <div v-else-if="field.type === 'objectList'">
    <template v-if="modelValue">
      <div
        v-for="(object, objectIndex) in modelValue"
      ...
      >
        <div v-if="!disabled" class="absolute text-right top-4 right-4">
          <button
            class="px-4 py-1 text-sm text-red-600 border border-red-600 rounded hover:bg-red-300"
        ...
        <div v-for="(objectField, index) in field.objectFields" class="my-2">
          ...
          <CustomInput v-model="object[objectField.key]" :field="objectField" :disabled="disabled" />
        </div>
      </div>
    </template>
    <button
      v-if="!disabled"
      class="w-full py-1 text-center border rounded border-primary hover:bg-primary/20 text-primary"
      @click="addObject"
    >
      Add
    </button>
  ...
  <div v-else-if="field.type === 'address'" class="flex">
    ...
    <div class="text-right w-22">
      <button
        class="px-4 py-1 border border-gray-400 rounded hover:bg-gray-200"
        @click="
          () => {
            isOpenFindAddressDialog = true;
          }
        "
      >
        주소 찾기
  ...
  <!-- 그 외 (text, date ..) -->
  <q-input
    v-else-if="field.type === 'text' || field.type === 'date' || field.type === 'url'"
    :type="field.type"
    :model-value="modelValue"
    @update:model-value="onUpdate"
...
const selectOptions: ComputedRef<Record<string, any>[]> = computed(() => {
  if (props.field.type !== 'select' || props.disabled) {
    return [];
  }
